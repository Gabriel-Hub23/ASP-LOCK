...
% default numeric domain (safe for the solver). If you want dynamic
% sizing, emit num/1 or explicit cell/2 facts from Python instead.
num(0..16).
...
:- chosen_move(D), adj(I,J,D,X,Y), locked(X,Y).
...
% costo della cella di arrivo per ogni mossa candidata
cand(D,N) :- next_cell(_,_,D,X2,Y2), dist(X2,Y2,N).

% distanza minima tra le mosse candidabili dalla cella corrente
min_step_cost(M) :- M = #min {N : cand(D,N)}.


% Weak constraint: minimizza la distanza della cella risultante dalla mossa scelta
% Peso = distanza (minimizzare), livello 1

penalizzazione(P) :-    chosen_move(D), 
                        next_cell(I,J,D,X,Y), 
                        #min{N: dist(X,Y,N)} = P.

:~ chosen_move(D), next_cell(I,J,D,X,Y), dist(X,Y,N). [N@2]
% Penalizza fortemente mosse verso celle irraggiungibili
:~ chosen_move(D), next_cell(I,J,D,X,Y), unreached(X,Y). [10@1]

% Weak constraint: penalizza mosse che riportano il giocatore sulla cella precedente (evita oscillazioni e movimenti inutili)
% Si disattiva se la penalizzazione + 1@2 pareggia il vincolo precedente
% Penalizza tornare alla cella precedente in caso di pareggio sul costo minimo
:~ chosen_move(D), adj(I,J,D,X,Y), prev(X,Y), min_step_cost(M), dist(X,Y,N), N = M. [1@2]

% Compatibilità API
take(D) :- chosen_move(D).


% il nemico deve mangiare il giocatore se è adiacente
adj_player(X,Y) :- player(X,Y), next_cell(I,J,_,X,Y).
:- not chosen_move(D), next_cell(I,J,D,X,Y), adj_player(X,Y).
